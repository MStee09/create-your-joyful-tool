...
interface ProductsListViewProps {
  ...
  /** Navigate to the newly created product after save */
...
export const ProductsListView: React.FC<ProductsListViewProps> = ({
  ...
  const filteredProducts = useMemo(() => {
    return [...productMasters]
      ...
      .map(product => {
        ...
        // Calculate effective price using pricing utility
    ...
    <HoverCard openDelay={300} closeDelay={100}>
      <HoverCardTrigger asChild>
        ...
        >
          ...
          {/* Pricing status with source indicator */}
          {product.pricingStatus === 'vendor' && product.effectivePrice && (
            <>
              <span className="text-muted-foreground/50">·</span>
              <span className="flex items-center gap-1 text-sm text-muted-foreground whitespace-nowrap">
                <Store className="w-3 h-3" />
                {formatCurrency(product.effectivePrice.price)}/{product.effectivePrice.unit}
              </span>
            </>
          )}
          {product.pricingStatus === 'estimated' && product.effectivePrice && (
            <>
              <span className="text-muted-foreground/50">·</span>
              <span className="flex items-center gap-1 text-sm text-amber-600 whitespace-nowrap">
          ...
          {product.hasVendor ? (
            <span className="text-xs text-emerald-600" title="Has vendor(s)">
              <CheckCircle2 className="w-3 h-3" />
            </span>
          ) : product.pricingStatus === 'estimated' ? (
            <span className="text-xs text-amber-600" title="No vendor, using estimated price">
              <AlertCircle className="w-3 h-3" />
            </span>
          ) : product.pricingStatus === 'none' ? (
            <span className="text-xs text-destructive" title="No vendor, no price">
  ...
  return (
    <div className="p-8">
      ...
      {/* Add Product Modal - New Simplified Single-Step */}
      <AddProductModal
        open={showAddModal}
